...
class FrequencyContainmentReserve(OperationStrategy):
    ...
    def next(self, time: float, system_state: SystemState, power: float = 0) -> float:
        ...
        p_out = power_fcr * self.__max_fcr_power
        self.__fcr_power = p_out
        self.__frequency_value = frequency
        return p_out
    ...
    def update(self, energy_management_state: EnergyManagementState) -> None:
        ...
        energy_management_state.fcr_power = self.__fcr_power
        energy_management_state.frequency = self.__frequency_value

    def clear(self) -> None:
        self.__power_fcr_last_step = 0.0

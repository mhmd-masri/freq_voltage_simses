...
class FFR4(OperationStrategy):
    ...
    def __init__(self, config: GeneralSimulationConfig, config_ems: EnergyManagementConfig, profile_config: ProfileConfig):
        ...
        self.__event_counter_FFR4 = 0  # Counter for frequency rate drops below 0.1 Hz/s
    ...
    def next(self, time: float, system_state: SystemState, power: float = 0) -> float:
        ...
        if frequency < self.__nominal_frequency:
            if abs(dfdt) > 0.1:
                if self.__counter ==0:
                    self.__t_start = time
                    self.__counter += 1
                    self.__event_counter_FFR4 += 1 # Increment the event counter

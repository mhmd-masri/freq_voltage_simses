...
class VoltageProfile(TechnicalProfile):

    def __init__(self, config: GeneralSimulationConfig, profile_config: ProfileConfig):
        super().__init__()
        self.__file: FileProfile = FileProfile(config, profile_config.voltage_file)
        #self.__file = open(profile_config.voltage_file, 'r', newline='')
        ...
        # self.__header = get_header_from(profile_config.voltage_file)

    def next(self, time) -> float:
        # Handling of profile files with and without header
        #if self.__header_missing_exception is True:
        #    __voltage = self.__first_line
        #    self.__header_missing_exception = False
        #else:
        #    __voltage = self.__file.readline()
        #return float(__voltage)

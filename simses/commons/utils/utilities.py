...
def get_standard_norm_for(data):
    """calculate the standard norm of the given data arrays
    when data are given in list, convert list into numpy data array
    """
    data_array = convert_to_numerical_array(data)
    sum_of_squares = np.sum(data_array**2, axis=1)

    return np.power(sum_of_squares, 0.5)


def convert_to_numerical_array(input_data):
    if type(input_data) == list:
        return np.array(input_data)
    elif isinstance(input_data, (int, float, complex)):
        return np.array([input_data])
    elif type(input_data) == np.ndarray:
        return input_data
    else:
        print('Input contains non-numeric data!')
